...
-- adapted by bontango
-- 8 bit version of rtl/common/slow_to_fast_clock.vhd
--
-- Taken from the WISOF project (version 0.9). It lives here and not in rtl/common
-- because the sound board is its only user: WISOF hands the DAC value over from the
-- sound PIA (cpu_clk) to the audio mixer (clk_50).
