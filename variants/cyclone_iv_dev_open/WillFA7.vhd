...
-- Ralf Thelen 'bontango' 07.2026
-- www.lisy.dev
--
-- v4.17 for dev_open Aliexpress board based on 3.17 for 'LISY' Cyclone IV dev4 board
-- ab v5.x eigene Board-ID, vorher kollidierte 4.17 mit der Cyclone-10-Variante
-- v5.18 switch matrix hardware debouncer (sw_debounce.vhd) against multiple switch triggers - per-switch, strobe-aware, incl. 2-FF sync
-- v5.19 special solenoid trigger: debounce time switchable via option DIP4 (57uS -> 250uS), against neighbour cross-triggering (sol 20 fired sol 19)
--       plus META_SPECIAL1..6 synchronizers moved from clk_50 to cpu_clk (target domain of spec_sol_trigger)
-- v5.20 switch matrix debounce masks selected by game number (sw_debounce, MASK_ROM)
--       plus FIX: LED_active ist die Blanking-Leitung und wird wieder ausschliesslich von
--       'blanking' getrieben. Behebt den kurzen Blanking-Einbruch ~6s nach Boot und bei
--       jedem EEprom-Save, der ab v4.17 auftrat.
...
entity WillFA7 is
	port(		
	   -- the FPGA board
		clk_50	: in std_logic; 	
		reset_sw  : in std_logic; 	-- S2-IO90 --goes Low on reset(push)
		LED_SD_Error : out STD_LOGIC;	-- LED4-D5 IO98
		LED_active 	: out STD_LOGIC;  -- LED3-D4 IO99
		LED_status 	: out STD_LOGIC;	-- LED2-D3 IO100
		LED_debug 	: out STD_LOGIC;	-- LED1-D2 IO101
...
-- SW version
constant SW_MAIN : std_logic_vector(3 downto 0) := x"5";
...
-- ACHTUNG: LED_active liegt hier zwar auf LED3-D4 (IO99) des Dev-Boards, fuehrt auf der
-- WillFA7-Platine aber die Blanking-Leitung: IC13 74HCT240 /OE (switch strobes) und ueber
...
-- umwidmen (Regression v4.17, siehe docs/blanking_led_active.md in CycloneIV_v4).
LED_active <= blanking;
...
-- Since v5.20 the mask is selected by game number, so every supported game gets its own
...
port map(
	clk           => cpu_clk,
	i_Rst_L       => reset_l,
	enable        => not game_option(5),  -- option DIP5 ON (game_option(5)='0') -> debounce; OFF -> raw v4.17 passthrough
...
port map(
   ...
   trigger => SPC_Sol_Trig_stable(1),
	...
	long_debounce => not game_option(4), -- DIP4 ON -> long debounce, OFF -> v5.18 behaviour
