...
-- v1.12 with memory protect and flipflop version 1.3 ( solves soundbug)
-- v1.13 with eeprom v094 which has reduced clock to 100KHz (old 1MHz)
-- v1.14 solenoid 17 with peak filter as we have 9uS peaks each 2mS on sp_solenoid_mpu(1);
-- v1.15 claude debug session: timing corrected, mem_clk confirmed, BT28 (SYS3 settings) corrected
-- v1.16 intermidiate with possible 'CONTACT' patch for special solenoid
-- v1.17 eeprom write robustnes (new SD_card and eeprom.vhd, SPI Master consolidated )
-- v1.18 switch matrix hardware debouncer (sw_debounce.vhd) against multiple switch triggers - per-switch, strobe-aware, incl. 2-FF sync
-- v1.19 special solenoid trigger: debounce time switchable via option DIP4 (57uS -> 250uS), against neighbour cross-triggering (sol 20 fired sol 19)
--       plus META_SPECIAL1..6 synchronizers moved from clk_50 to cpu_clk (target domain of spec_sol_trigger)
-- v1.20 switch matrix debounce masks selected by game number (sw_debounce, MASK_ROM)
...
--       ~6s nach Boot und bei jedem EEprom-Save, der ab v1.17 auftrat.
--       ROM-Bereich bleibt bei 5 ROMs (10 KByte) - das ist die Cyclone-II-Eigenheit.
--       plus SPECIAL1..6 bekommen jetzt 'i_Rst_L => reset_l' statt der festen '1'. Bisher wurde
--       ein laufender Solenoid-Puls bei Reset nicht abgeworfen; alle anderen Varianten machen das
--       seit langem so. Damit ist der .20-Stand auch hier vollstaendig.
...
-- umwidmen (Regression v1.17, siehe docs/blanking_led_active.md in CycloneIV_v4).
LED_active <= blanking;
...
SD_CARD: entity work.SD_Card
			generic map(
	Read_Bytes => 10240  -- 10 KByte = 20 sectors per game (5 ROMs x 2K)
) 
port map(
	i_clk		=> clk_50,	
	-- Control/Data Signals,
...
-- Since v1.20 the mask is selected by game number, so every supported game gets its own
...
port map(
	clk           => cpu_clk,
	i_Rst_L       => reset_l,
	enable        => not game_option(5),  -- option DIP5 ON (game_option(5)='0') -> debounce; OFF -> raw v1.17 passthrough
...
--sp_solenoid(0) <= sp_solenoid_trig(1) and GameOn; -- to prevent activation
...
port map(
   ...
   trigger => SPC_Sol_Trig_stable(1),
	...
	long_debounce => not game_option(4), -- DIP4 ON -> long debounce, OFF -> v1.18 behaviour
