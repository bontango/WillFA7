...
-- v4.15 Cyclone 10
-- v4.16 intermidiate with possible 'CONTACT' patch for special solenoid
-- v4.17 eeprom write robustnes (new SD_card and eeprom.vhd, SPI Master consolidated )
-- v4.18 switch matrix hardware debouncer (sw_debounce.vhd) against multiple switch triggers - per-switch, strobe-aware, incl. 2-FF sync
-- v4.19 special solenoid trigger: debounce time switchable via option DIP4 (57uS -> 250uS), against neighbour cross-triggering (sol 20 fired sol 19)
--       plus META_SPECIAL1..6 synchronizers moved from clk_50 to cpu_clk (target domain of spec_sol_trigger)
-- v4.20 switch matrix debounce masks selected by game number (sw_debounce, MASK_ROM)
...
--       ~6s nach Boot und bei jedem EEprom-Save, der ab v4.17 auftrat.
...
-- umwidmen (Regression v4.17..v4.19, siehe docs/blanking_led_active.md in CycloneIV_v4).
LED_active <= blanking;
...
-- Since v4.20 the mask is selected by game number, so every supported game gets its own
...
port map(
   ...
   trigger => SPC_Sol_Trig_stable(1),
	...
	long_debounce => not game_option(4), -- DIP4 ON -> long debounce, OFF -> v4.18 behaviour
